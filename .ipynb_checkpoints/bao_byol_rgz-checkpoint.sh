#!/bin/sh
#SBATCH --job-name byol          # this is a parameter to help you sort your job when listing it
#SBATCH --error /home/users/l/lastufka/sbatch_logs/byol-error.e%j     # optional. By default a file slurm-{jobid}.out will be created
#SBATCH --output /home/users/l/lastufka/sbatch_logs/byol-out.o%j      # optional. By default the error and output files are merged
...
PYTHONPATH=/home/users/l/lastufka/PyTorch-BYOL:$PYTHONPATH
...
OUT_DIR="/home/users/l/lastufka/scratch/byol_rgz/"

#if [ ! -d "$OUT_DIR" ]; then
    mysrun -m accelerate.commands.launch --num_processes 2 /home/users/l/lastufka/eupe_paper/byol_pt.py --output_dir $OUT_DIR \
    --dataset RGZ #--fake_chan
        # --model vit_small \
        # --lr $LR \
        # --output_dir $OUT_DIR \
        # --batch_size 64 \
